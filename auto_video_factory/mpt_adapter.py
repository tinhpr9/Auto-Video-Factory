...
def build_config_toml(
    *,
    ...
    """
    pexels_list = f'["{pexels_api_key}"]' if pexels_api_key else "[]"
    pixabay_list = f'["{pixabay_api_key}"]' if pixabay_api_key else "[]"

    # LLM provider section — only openai supported in initial baseline
    if llm_provider == "openai":
        llm_section = f'openai_api_key = "{llm_api_key}"\nopenai_model_name = "gpt-4o-mini"'
    else:
...
def locate_output_video(mpt_root: str, task_id: str) -> str:
    ...
    # Also check non-recursive for flat task dirs
    if not candidates:
        candidates = glob.glob(os.path.join(task_dir, "*.mp4"))
...
def sanitize_result_metadata(raw: dict[str, Any]) -> dict[str, Any]:
    """
    ...
    Args:
        raw: Raw metadata dict (possibly from cli.py JSON output or config).

    Returns:
        New dict safe to upload as a workflow artifact.
    """
